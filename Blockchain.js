...
class Blockchain{
    ...
    async addBlock(newBlock){
        ...
        let initial_recall_data = {
            recall_id:(Math.floor(Math.random() * Number.MAX_SAFE_INTEGER)) % this.total_chain_height,
            //flag:false,
        };
        newBlock.recall_arr.push(initial_recall_data);
        recal_arr_size--;
        ...
        lbl: while (recal_arr_size > 0) {
            ...
            for (let i = 0; i < newBlock.recall_arr.length; i++) {
                if(recall_block_height === newBlock.recall_arr[i].recall_id ){
                    // continue lbl;
                    flag = false;
                }
            }
        ...
        console.log("curr h: "+this.total_chain_height+" Logarithm size: " + newBlock.recall_arr.length + " recall_array: " + JSON.stringify(newBlock.recall_arr) )
